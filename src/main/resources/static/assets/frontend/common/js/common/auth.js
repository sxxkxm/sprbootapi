 ...
 *  
 */
export var userLocalAccessToken = "";

export function userGetAccessToken(callbackSettings){
			...
			success: (data, status, xhr)=>{
	        	switch(xhr.status){
		    		case constants.HttpStatusCodes.OK:
		    			userLocalAccessToken = xhr.getResponseHeader("Authorization");
		    			if(callbackSettings != null){
		    				callbackSettings.headers = {"Content-Type": "application/json", "Authorization": userLocalAccessToken},
 ...
 */
export function userCheckAuth(roleType){
	return new Promise((resolve, reject) => {
		$.ajax({
			url: "/api/auth/role-type/"+roleType,
			method: "POST",
			cache: false,
			headers: {"Content-Type": "application/json", "Authorization": userLocalAccessToken},
			success: (data, status, xhr)=>{
	        	switch(xhr.status){
		    		case constants.HttpStatusCodes.OK:
		    			break;
